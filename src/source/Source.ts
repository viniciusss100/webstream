...
export abstract class Source {
  ...
  public async handle(ctx: Context, type: ContentType, id: Id): Promise<(SourceResult[])> {
    ...
    require('fs').appendFileSync('/tmp/videasy-debug.log', `[${this.id}] cache hit: ${sourceResults !== undefined}, key: ${cacheKey}\n`);
    ...
    if (!sourceResults) {
      ...
      await sourceResultCache.set<SourceResult[]>(cacheKey, sourceResults, sourceResults.length > 0 ? this.ttl : 60000);
    }
